...
public class Dither implements Filter{
    ...
    public BufferedImage execute(BufferedImage image) {
        ...
        int levelRed = 255 / (quantizationNumberRed - 1);
        int levelGreen = 255 / (quantizationNumberGreen - 1);
        int levelBlue = 255 / (quantizationNumberBlue - 1);
        ...
        for(int x = 0; x < image.getWidth(); ++x) {
            for (int y = 0; y < image.getHeight(); ++y) {
                ...
                int newR = (int)(R + levelRed * ((matrixRed[x % matrixRed.length][y % matrixRed.length] /
                        (matrixRed.length * matrixRed.length) - 0.5)));

                int newG =  (int)(G + levelGreen * ((matrixGreen[x % matrixGreen.length][y % matrixGreen.length] /
                        (matrixGreen.length * matrixGreen.length) - 0.5)));

                int newB =  (int)(B + levelBlue * ((matrixBlue[x % matrixBlue.length][y % matrixBlue.length] /
                        (matrixBlue.length * matrixBlue.length) - 0.5)));

                newR = getNearestColor(newR, levelRed);
                newG = getNearestColor(newG, levelGreen);
                newB = getNearestColor(newB, levelBlue);
    ...
    private int getNearestColor(double color, int quantizationNumber){
        color = Math.round(color / quantizationNumber) * quantizationNumber;
    ...
    private double[][]getMatrix(int quantizationNumber){
        for(int i = 1; i <= 4; ++i){
            ...
            if(sizeMatrix * sizeMatrix >= 256 / quantizationNumber){
